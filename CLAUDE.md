...
정본 문맥설계는 `docs/`에 역할별로 있다 — 제품 `docs/user/`, 개발 `docs/dev/`, 디자인 `docs/design/`, 보안 `docs/security/`, 결정·규칙 `docs/agent/`(`ADR.md`·`RULES.md`·`STATE.md`). 충돌 시 루트 `README.md`·`PRD.md`·이 파일을 따른다.
...
- **CRITICAL: 비밀은 `.env`(서버)에서만.** `NEXT_PUBLIC_*`에 키 금지, 클라 번들 노출 금지. 외부 키가 없어도 build/test는 통과해야 하고(미설정 기능은 503으로 안내), `.env` 등 비밀 파일은 직접 읽지 않는다(차단 훅 존재). (R2)
- 컴포넌트는 `components/`, 타입은 `types/`, 서버 유틸은 `lib/`, 백그라운드 잡은 `worker/`. 역할·상태 등은 enum 대신 `String` + zod 검증.

## 개발 프로세스
- **CRITICAL: TDD.** 새 기능·버그픽스는 테스트를 먼저 쓰고(RED) 통과하는 구현을 작성한다(GREEN). 위치: `lib/__tests__`·`app/api/**/__tests__`·`components/**/__tests__`.
