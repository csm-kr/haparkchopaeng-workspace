...
> **프로덕션 영상 = LiveKit(SFU).** (ADR-019, Cloudflare Stream Live 대체) 영상 인프라를 직접 만들지 않고, 앱은 **입장 토큰 발급 · 권한 체크 · 룸 정리**만 담당한다. 라이브 시작 시 LiveKit 룸을 만들고 발표자에게 입장 토큰(화면공유 grant 포함)을 발급하며, 참가자는 `/join`으로 토큰을 받아 같은 룸에 접속한다(카메라는 선택 — 끄면 아바타 타일). 모델은 **다자간 실시간 화상 컨퍼런스**(서로 얼굴 보는 세미나)로 확정 — 채팅·반응·손들기는 LiveKit 데이터 채널.
