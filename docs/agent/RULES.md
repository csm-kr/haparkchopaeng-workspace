...
- **R18. 팀 합류는 초대 토큰으로만.** 공개 가입(자동 팀 배정) 금지. 로그인은 누구나 가능(처음엔 팀 없음)하되, 팀 진입은 **팀 생성** 또는 **초대 토큰 수락**으로만. 팀 역할 `owner`/`admin`/`member`(`owner`는 초대 불가 · 팀당 ≥ 1). 전역 팀 상한은 `AppSetting.maxTeams` > env `MAX_TEAMS` > 기본 2 순으로 정해지며 `/admin`에서 조절한다(ADR-023). 기존 단일 워크스페이스 기능의 `Member.role`(관리자/멤버/게스트) 권한은 팀 스코핑 전까지 유지. (ADR-007→018, ADR-016)
