...
| `docs/agent/ADR.md` (수정) | ADR-021 개정 + ADR-024 신규 |
...
```prisma
// 전역 앱 설정 (ADR-024). 관리자 콘솔(/admin)이 조절한다.
...
// 전역 앱 설정 (서버 전용, ADR-024). 관리자 콘솔(/admin)이 조절한다.
...
```ts
// CRITICAL: 전역 상한은 lib/settings.ts가 소유한다(DB > env > 2, ADR-024). 여기서 env를 직접 읽지 마라.
```
...
// 전역 팀 상한 조절 폼(ADR-024). 서버가 최종 강제 — 여기 검증은 가시화일 뿐이다.
...
```markdown
> **2026-08-01 개정(ADR-024):** "admin이 env(`MAX_TEAMS`)로만 설정" 조항은 폐기됐다. 상한은 이제 DB(`AppSetting.maxTeams`)에 저장되고 관리자 콘솔(`/admin`)에서 조절한다. env `MAX_TEAMS`는 DB row가 없을 때의 폴백으로만 남는다. **전역 상한이라는 성격(per-user 아님)은 그대로다.**
```
...
```markdown
## ADR-024. 관리자 콘솔(`/admin`) — 전역 설정 + 라이브 사용량 가시화
...
```markdown
전역 팀 상한은 `AppSetting.maxTeams` > env `MAX_TEAMS` > 기본 2 순으로 정해지며 `/admin`에서 조절한다(ADR-024).
```
...
```markdown
| `MAX_TEAMS` | 전역 팀 생성 상한 **폴백** | 기본 `2`. `AppSetting.maxTeams`가 있으면 그쪽이 이긴다(ADR-024) |
...
git commit -m "docs(admin): ADR-024 추가 + ADR-021 상한 정책 개정 + ENV 3항목 문서화"
```
...
- Vercel 프로덕션에 `MAX_TEAMS=3`은 **이미 설정돼 있다**(2026-08-01). ADR-024 적용 후에도 DB row가 없으면 이 값이 폴백으로 쓰이므로 지울 필요 없다.
