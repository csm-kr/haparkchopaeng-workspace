...
import { PresentationTitleEditable } from "../presentation-title";

// 발표 자료 이름(제목) 인라인 편집 — 연필 버튼으로 입력창 열고 PATCH로 저장, 성공 시 화면 갱신.
...
describe("PresentationTitleEditable", () => {
  it("기본은 제목을 h1로 보여준다", () => {
    render(<PresentationTitleEditable presentationId="pres1" initialTitle="원래 제목" />);
  ...
  it("저장하면 PATCH로 새 제목을 보내고 화면을 갱신한다", async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ data: { id: "pres1", title: "바뀐 제목" } }),
    });
  ...
  it("저장 시 앞뒤 공백을 잘라서 보낸다", async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ data: { id: "pres1", title: "다듬은" } }),
    });
    vi.stubGlobal("fetch", fetchMock);
