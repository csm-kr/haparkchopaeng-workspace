...
describe("seed counts", () => {
  ...
  it("논문 5건 — p5는 분석 없음(pending)", async () => {
    expect(await prisma.paper.count()).toBe(5);
    expect(await prisma.paper.count({ where: { analysisStatus: "ready" } })).toBe(4);
    expect(await prisma.paper.count({ where: { analysisStatus: "pending" } })).toBe(1);
  });

  it("분석은 두 관점뿐 — research/repro만 (ADR-004)", async () => {
    expect(await prisma.analysis.count()).toBe(8); // p1~p4 × {research, repro}
    const lenses = new Set((await prisma.analysis.findMany({ select: { lens: true } })).map((a) => a.lens));
    expect([...lenses].sort()).toEqual(["repro", "research"]);
  });

  it("figures + section notes", async () => {
    expect(await prisma.figure.count()).toBe(6);
    expect(await prisma.sectionNote.count()).toBe(5);
    // figures 노트는 항상 any (ADR-005)
    const figureNotes = await prisma.sectionNote.findMany({ where: { sectionId: "figures" } });
    expect(figureNotes.every((n) => n.lens === "any")).toBe(true);
  });

  it("발표 자료 3건 + 댓글", async () => {
    expect(await prisma.presentation.count()).toBe(3);
