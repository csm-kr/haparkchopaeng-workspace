...
아래는 실제 구동 중인 앱(**모두의모임 / 하박조팽** 워크스페이스)의 화면이다. 브랜드 카피는 **모두의모임(모모)**, 코드·DB·URL 식별자는 `hapark`/하박조팽으로 유지한다.

### 홈 (대시보드)

논문·발표 자료 퀵 카드, 최근 활동, 현재 팀과 멤버 목록.

![홈 대시보드](docs/assets/screenshots/dashboard.png)

### 세미나 스케쥴 · 로테이션 · 벌금

매주 토요일 발표 순번, 발표자 로테이션, 발표자/일반 불참 벌금 설정.

![세미나 스케쥴](docs/assets/screenshots/schedule.png)

### 논문 목록 & 발표 자료

업로드된 논문(연구/재구현 두 관점 분석)과 슬라이드 수가 함께 쌓이는 발표 자료.

![논문 목록](docs/assets/screenshots/library.png)

![발표 자료](docs/assets/screenshots/presentations.png)

### 세미나 라이브

LiveKit 기반 다자간 화상 룸. 라이브가 없을 때의 빈 상태 — 시작하면 모두에게 알림이 가고 룸이 열린다.

![세미나 라이브](docs/assets/screenshots/meeting.png)
